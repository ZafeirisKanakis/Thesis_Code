        ...
        with tqdm.tqdm(total=self.gan_epochs) as pbar:
            for i_epoch in range(self.gan_epochs):
                ...
                for data_item in input_data:
                    ...
                    # Plot the original and noisy images (only for the first iteration)
                    # Plot the original and noisy images (only for the first iteration)
                    if i_iter == 1:  # Plot only for the first iteration
                        noisy_img = fake_feats[0].detach().cpu().numpy()

                        # Print the shape of noisy_img to understand its dimensions
                        print(f"Shape of noisy_img before reshape: {noisy_img.shape}")

                        # Check possible reshapes based on size
                        try:
                            if noisy_img.size == 1536:
                                # Grayscale reshape: 1 channel with 48x32 resolution
                                noisy_img = noisy_img.reshape(1, 48, 32)
                                print(f"Shape of noisy_img after reshape (Grayscale): {noisy_img.shape}")
                            elif noisy_img.size == 3072:
                                # RGB reshape: 3 channels with 32x32 resolution
                                noisy_img = noisy_img.reshape(3, 32, 32)
                                print(f"Shape of noisy_img after reshape (RGB): {noisy_img.shape}")
                            else:
                                raise ValueError(f"Cannot reshape array of size {noisy_img.size} into a valid shape.")

                        except ValueError as e:
                            print(f"Error during reshape: {e}")
                            continue  # Skip this iteration if reshaping fails

                        # Normalize or clip noisy_img to be within the expected range [0, 1] for visualization
                        noisy_img = np.clip(noisy_img, 0, 1)

                        # Normalize the image (if it's not in the expected range, you can try this)
                        if noisy_img.max() > 1:
                            noisy_img = noisy_img / 255.0  # Normalize if the max value is greater than 1

                        # Plot the images using matplotlib
                        plt.figure(figsize=(10, 5))

                        # Original Image Plot
                        original_img = img[0].detach().cpu().numpy()  # Fetch the original image
                        # Transpose original_img from (C, H, W) to (H, W, C) for RGB
                        original_img = original_img.transpose(1, 2, 0) if original_img.shape[0] == 3 else original_img[0].transpose(1, 2, 0)
                        ...
                        # Noisy Image Plot
                        noisy_img = noisy_img.transpose(1, 2, 0)  # Transpose from (C, H, W) to (H, W, C)
                        plt.subplot(1, 2, 2)
                        plt.imshow(noisy_img, cmap="gray" if noisy_img.shape[-1] == 1 else None)
                        plt.title("Noisy Image")
                        plt.axis("off")

                        # Save the plot
                        plt.savefig(f'output_{i_iter}.png')
                        plt.close()
